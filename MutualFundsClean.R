setwd("~/Desktop/Grad School/Stat 9890/Project/MutualFunds")
...
### split the dataset into training and test set with the split ratio creates a value for dividing the data into train and test. 
#In this case the value is defined as 80% of the number of rows in the dataset

#Cleaning Data
sum(is.na(MutualFundsCleaned))
#anyNA(MutualFundsCleaned)
#MutualFundsCleaned[is.na(MutualFundsCleaned$price_cashflow_ratio)]=0
#MutualFundsCleaned[MutualFundsCleaned == Inf] <- 0
#MutualFundsCleaned <- MutualFundsCleaned %>% mutate_if(is.character,as.factor)
#is.nan.data.frame <- function(x) do.call(cbind, lapply(x, is.nan))
#MutualFundsCleaned[is.nan(MutualFundsCleaned)] <- 0
#names(MutualFundsCleaned)[16]="fund_return_ytd"
# str(MutualFundsCleaned)
# table(MutualFundsCleaned$fund_symbol)
# nlevels(MutualFundsCleaned$fund_symbol)
# table(MutualFundsCleaned$fund_family)
# nlevels(MutualFundsCleaned$fund_family)
# table(MutualFundsCleaned$investment_type)
# nlevels(MutualFundsCleaned$investment_type)
...
# creating X matrix
#X=data.matrix(Housing%>%select(-LotArea))   # decided to choose another y
x=data.matrix(MutualFundsCleaned%>%select(-fund_return_2019))
dim(x)

#distribution check

hist(MutualFundsCleaned$fund_return_2019, xlim = range(-20,60))
...
#rsq_rid_cv=cor(y_full, y_hat_cv_full)^2  another way to check Rsq
